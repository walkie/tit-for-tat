...
pub type Profile<Move, const NUM_PLAYERS: usize> = PerPlayer<Move, NUM_PLAYERS>;

/// The most general form of simultaneous move game.
///
/// This representation is best used for games with non-finite domains of moves. For games with
/// finite domains of moves, use [`Normal`](crate::game::Normal).
///
/// # Type variables
...
/// ```
pub struct Simultaneous<Move, Utility, const NUM_PLAYERS: usize> {
    move_fn: Box<dyn Fn(PlayerIndex<NUM_PLAYERS>, &Move) -> bool>,
    payoff_fn: Box<dyn Fn(&Profile<Move, NUM_PLAYERS>) -> Option<Payoff<Utility, NUM_PLAYERS>>>,
}

impl<Move, Utility, const NUM_PLAYERS: usize> Simultaneous<Move, Utility, NUM_PLAYERS> {
    ...
    pub fn new(
        move_fn: impl Fn(PlayerIndex<NUM_PLAYERS>, &Move) -> bool + 'static,
        payoff_fn: impl Fn(&Profile<Move, NUM_PLAYERS>) -> Option<Payoff<Utility, NUM_PLAYERS>> + 'static,
    ) -> Self {
        Simultaneous {
    ...
    pub fn is_valid_profile(&self, profile: &Profile<Move, NUM_PLAYERS>) -> bool {
    ...
    pub fn payoff(&self, profile: &Profile<Move, NUM_PLAYERS>) -> Option<Payoff<Utility, NUM_PLAYERS>> {
    ...
    pub fn law_valid_profile_yields_payoff(&self, profile: &Profile<Move, NUM_PLAYERS>) -> bool {
